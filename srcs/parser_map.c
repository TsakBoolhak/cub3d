...
int	check_around_cell(int x, int y, t_cub *cub)
{
	if (x < cub->map_w - 1 && !ft_strchr("021 NEWS", cub->map[y][x + 1]))
		return (return_error("MAP ERROR: Invalid character in map\n", -1));
	...
	if (x < cub->map_w - 1 && !ft_strchr(" 1", cub->map[y][x + 1]))
		return (return_error("MAP ERROR: Map is open\n", -1));
...
int	set_sprite_position(int x, int y, t_cub *cub)
{
	...
	if (!sprites)
		return (return_error("MEM ERROR: Couldnt allocate memory\n", -1));
	sprites[i + 1] = NULL;
...
int	ft_check_map_cell(int x, int y, t_cub *cub, int *spawn)
{
	...
	{
		if (*spawn)
			return (return_error("MAP ERROR: two many spawn points\n", -1));
		*spawn = 1;
		set_spawn(x, y, cub);
	}
...
int	map_is_valid(t_cub *cub)
{
	...
	if (!spawn)
		return (return_error("MAP ERROR: there is no spawn in the map!\n", -1));
	return (0);
}
